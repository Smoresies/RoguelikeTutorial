...
# Max item per room on floor. [1,1] means 1 from level 1 on, until next given.
# So [4,2] means from 4 onwards you can get 2. Replacing the [1,1]
const max_items_by_floor = [
	[1, 1],
	[4, 2]
]

# Same logic as max_items
...
# First number is floor. As gets bigger the weight for items changes
# Weight works like giving more raffle tickets to each, not as % of 100
const item_chances = {
	0: {"health_potion": 35},
...
# Uses the max_items/enemies_per_floor arrays to determine max value for floor
func _get_max_value_for_floor(weighted_chances_by_floor: Array, current_floor: int) -> int:
	...
	for chance in weighted_chances_by_floor:
		# chance[0] is the floor
		if chance[0] > current_floor:
			break
		else:
			# chance[1] is the max # of elements for this floor
