...
func setup(player: Entity) -> void:
	...
	health_upgrade_button.text = "Constitution (+20 HP, from %d)" % fighter.max_hp
	power_upgrade_button.text = "Strength (+1 attack, from %d)" % fighter.power
	defense_upgrade_button.text = "Agility (+1 defense, from %d)" % fighter.defense
